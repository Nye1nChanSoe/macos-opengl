...
using ShaderLayoutName = const std::string &;
...
struct BufferElement
{
    ...
    BufferElement(BufferAttributeType attributeType, ShaderLayoutName name, Normalized isNormalized = false)
...
public:
        ...
        : m_Stride(0)
    {
        ...
        {
            m_Offsets.push_back(m_Stride * 4);
            Logger::Debug("Offset: {}", m_Stride * 4);
            m_Stride += bufferElement.m_Count;
        }
        Logger::Debug("Stride: {}", m_Stride);
    }

    inline GLsizei GetStride() const { return m_Stride; }
    inline const std::vector<unsigned int> &GetOffsets() const { return m_Offsets; }
};
